...
do_propegate_message(State, Ref, ClientPID, Message) ->
	...
	NotIncluded = maps:keys(maps:remove(ClientPID, State#chat_st.registrations)),
	lists:map(fun(X) -> X!{request, self(), Ref, {incoming_msg, CliNick, State#chat_st.name, Message}} end, NotIncluded),
	#chat_st{
		name = State#chat_st.name,
